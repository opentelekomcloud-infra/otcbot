...
from otcbot.bot_commands import Command
from otcbot.chat_functions import make_pill, react_to_event, send_text_to_room
from otcbot.config import Config
from otcbot.message_responses import Message
...
class Callbacks:
    ...
    async def message(self, room: MatrixRoom, event: RoomMessageText) -> None:
        ...
        if not has_command_prefix and room.member_count > 2:
            ...
            message = Message(
                self.client, self.store, self.config, msg, room, event
            )
    ...
    async def _reaction(
        ...
        event_response = await self.client.room_get_event(
            room.room_id, reacted_to_id
        )
        ...
        message = f"{reaction_sender_pill} reacted to this event with `{reaction_content}`!"
    ...
    async def unknown(self, room: MatrixRoom, event: UnknownEvent) -> None:
        ...
        if event.type == "m.reaction":
            ...
            relation_dict = event.source.get("content", {}).get(
                "m.relates_to", {}
            )
